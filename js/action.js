(function($, window, undefined) {
    ...
    function markerClickHandle(e) {
        ...
        config.timer = setInterval(function() {
            ...
            console.log('auto', timer_sec);
        }, 500);
        var loc = findMarkerFromList(this, markers);
    ...
    function submitUser(e) {
        ...
        $.post('api/action.aspx', form.serialize(), function(data) {
            connecting = false;
    ...
    function delegateListener() {
        ...
        $('#content .right-button.browse a').click(function() {
        ...
        $('#gallery .share a.btn').hover(function() {
            ...
            if ($(this).parent().hasClass('submit')) {
                ...
                $('#gallery .share textarea').val(status + " " + window.location.href);
                connecting = true;
                $.post(config.api_path, $('#gallery .share form').serialize(), function(data) {
                    ...
                    if (data.code == 0) {
                        ...
                        alert("请先绑定你的微博账号。");
                        window.open("OAuth.aspx", "sina_oauth", "");
                    } else {
                        alert(data.msg)
                    }
                });
    ...
    $(function() {
        nsd.data = {};
        nsd.user.token = getCookie("auth_token");
        adjust();
